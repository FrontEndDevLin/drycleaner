    ...
    this.OnStatist = function (req, res, handle) {
        ...
        switch (handle) {
            case 'getstatist': {
                ...
                let top6CommDay = param["top6CommDay"], top5StaffDay = param["top5StaffDay"], top3StoreDay = param["top3StoreDay"];
                ...
                // console.log(top6CommDay, top5StaffDay, top3StoreDay);
                // 今日前三衣物
                let now = new Date();
                let progress = 0;
                let rspData = { top6Sell: [], top3Store: [], top5Staff: [] };
                    var dayStart = top6CommDay ? new Date(top6CommDay.toLocaleDateString()).getTime() : new Date(now.toLocaleDateString()).getTime();
